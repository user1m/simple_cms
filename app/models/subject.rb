class Subject < ActiveRecord::Base
	...
	scope :visible, lambda {where(:visible => true)} #finding all visible subjects
	scope :invisible, lambda { where(:visible => false)} # acll Subject.invisible
	scope :sorted, lambda {order("subjects.position ASC")}
	scope :newest_first, lambda {order("subjects.created_at DESC")}
	scope :search, lambda {|query|
			where(["name LIKE ?", "%#{query}%"]) #matches query as a substring
		}
	end
